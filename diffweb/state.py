...
class State:
    def __init__(self, db_path: Path):
        ...
        # One connection shared by request-pool threads and background PR
        # refreshes. sqlite3 connections are not safe for concurrent use even
        # with check_same_thread=False - interleaved calls raise
        # "bad parameter or other API misuse" - so every access takes the lock,
        # reads included. The queries are tiny, so contention is not a concern.
    ...
    def base_ref(self, worktree: str) -> str | None:
        with self._lock:
            row = self._conn.execute(
                "SELECT base_ref FROM settings WHERE worktree = ?", (worktree,)
            ).fetchone()
    ...
    def reviewed(self, worktree: str) -> dict[str, str]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT path, blob_sha FROM reviewed WHERE worktree = ?", (worktree,)
            ).fetchall()
    ...
    def unmark_reviewed(self, worktree: str, path: str) -> None:
        with self._lock:
            self._conn.execute(
                "DELETE FROM reviewed WHERE worktree = ? AND path = ?", (worktree, path)
            )
            self._conn.commit()
